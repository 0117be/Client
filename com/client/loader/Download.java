...
public class Download {
	...
	private void visitSite() throws IOException {
		if(offlineTest) {
			String url = "http://revelation-rsps.com/Revelation.jar";
			URL u = new URL(url);
			...
			while ((read = in.readLine()) != null) {
				...
				FileOutputStream out = new FileOutputStream("./Revelation.jar");
				...
				while ((count = inn.read(b)) > 0) {
					out.write(b, 0, count);
					down += count;
					Loader.drawLoadingText(percentage(down, length), "Downloading Revelation - " + percentage(down, length) + "%");
				}
				out.close();
				inn.close();
			}
